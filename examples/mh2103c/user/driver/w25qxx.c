...
void w25qxxRead_1bitDma(uint8_t* pBuffer,uint32_t ReadAddr,uint16_t NumByteToRead)
{
    ...
    spiDmaReadWrite(W25QXX_SPI_X,tempBuf,cmdBuf,4);
    spiDmaRead(W25QXX_SPI_X,pBuffer,NumByteToRead);
    W25QXX_CS_OFF;
}

void w25qxxReadFast_1bitDma(uint8_t* pBuffer,uint32_t ReadAddr,uint16_t NumByteToRead)
{
    uint8_t cmdBuf[5];
    uint8_t tempBuf[5];
    W25QXX_SET_SPEED_MAX;
    W25QXX_CS_ON;
    cmdBuf[0]=W25X_FastReadData;
    cmdBuf[1]=(ReadAddr >> 16) & 0xFF;
    cmdBuf[2]=(ReadAddr >> 8) & 0xFF;
    cmdBuf[3]=ReadAddr& 0xFF;
    cmdBuf[4]=0xFF;
    spiDmaReadWrite(W25QXX_SPI_X,tempBuf,cmdBuf,5);
    spiDmaRead(W25QXX_SPI_X,pBuffer,NumByteToRead);
    W25QXX_CS_OFF;
}
#endif 
...
//    w25qxxRead_1bitDma(pBuffer,ReadAddr,NumByteToRead);
    w25qxxReadFast_1bitDma(pBuffer,ReadAddr,NumByteToRead);
#else
    w25qxxReadFast_1bit(pBuffer,ReadAddr,NumByteToRead);
//    w25qxxRead_1bit(pBuffer,ReadAddr,NumByteToRead);
#endif 
}
