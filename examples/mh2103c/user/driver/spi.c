...
void spiDmaWrite(SPI_TypeDef* SPIx,uint8_t *pWriteData,uint16_t length)
{
    ...
    DMA1_Channel3->CMAR = (uint32_t)pWriteData;    //设置发送数据的源SRAM地址

    DMA1_Channel3->CNDTR= length;    //设置发送字节长度,发送SRAM地址不增加

    DMA1_Channel2->CMAR =(uint32_t)(&dmaFF); //设置接收数据个数

    DMA1_Channel2->CNDTR= length;    //设置接收数据的目标SRAM地址
...
//    }
    ...
    DMA1_Channel3->CMAR = (uint32_t)(&dmaFF);    //设置发送数据的源SRAM地址

    DMA1_Channel3->CNDTR= length;    //设置发送字节长度,发送SRAM地址不增加

    DMA1_Channel2->CMAR =(uint32_t)pReadData; //设置接收数据个数
